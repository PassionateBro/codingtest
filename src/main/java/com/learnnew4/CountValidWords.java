...
/**
 * @describtion: 2047. 句子中的有效单词数
 * @author: CaiJm
 * @Date: 2022-01-27 9:22
 */
public class CountValidWords {
    ...
    private boolean judge(String word) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (chars[i] == '-') {
                if (connect == 0 || i == 0 || i == chars.length - 1) return false;
                if (chars[0] < 'a' || chars[0] > 'z') return false;
                if (i == chars.length - 2 && !(chars[chars.length - 1] >= 'a' && chars[chars.length - 1] <= 'z'))
                    return false;
                connect--;
                continue;
            }
            if (chars[i] == '!' || chars[i] == '.' || chars[i] == ',') {
                if (dot == 0 || i != chars.length - 1) return false;
                if (i == 0) return false;
                dot--;
                continue;
            }
    ...
    public static void main(String[] args) {
        ...
        countValidWords.countValidWords(s);
    }
}
